...
创建一个名为server.js的文件，文件就在node-demo-1里面。
...
​                                 curl http://127.0.0.1:8888/xxxx`

​                                 作用：向8888端口发送请求（也可以通过chrome发请求）

此时，server.js的终端会反映一个：

​                                 有个帅哥发请求过来啦！路径（带查询参数）为：8888`

​                                  作用：说明server.js文件在监听8888，做出了响应

注意：server.js文件中有句反单引号哈哈，``这种里面可以直接进行回车，' '单引号里面只能用\n表示
...
​      `console.log("有人访问/了");`/**server.js文件反应监听的恢复**/

​      `response.setHeader('Content-Type', 'text/html;charset=utf-8');`/**表示响应内容的类型or语法**/

​      response.write(`fucking crazy\n`)/**表示响应的内容，此处有反单引号，``这种里面可以直接进行回车，' '单引号里面回车只能用\n表示**

​      `response.end();`**表示响应可以发给用户了**

  `}`

以上为定义路径响应字符串的套路 else if 接上面内容可以继续使用其他路径返回字符创。  请求响应的原理就是这样了。
...
​              `response.statusCode = 404` **未知路径，返回404。当请求为未知内容，则按照404旗下的相应内容来响应**
